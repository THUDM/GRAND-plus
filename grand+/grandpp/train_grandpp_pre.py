...
class Grand_MLP_Post_Loss(nn.Module):
    def __init__(self, num_features, num_classes, hidden_size, nlayers, use_bn, dropout):
        super().__init__()
        self.mlp = MLP(num_features, hidden_size,
                       hidden_size, nlayers - 1, use_bn, dropout)
        self.linear = MLP(hidden_size, num_classes, hidden_size, 1, use_bn, dropout)
        self.dropout = nn.Dropout(dropout)
        self.bn = nn.BatchNorm1d(hidden_size)
        self.use_bn = use_bn
    def forward(self, X):
        logits = F.relu(self.mlp(X))
    ...
    def random_prop(self, logits, ppr_scores, ppr_idx):
        ...
        ppr_sum_s = scatter(ppr_scores[:,None], ppr_idx[:,None],
                                    dim=0, dim_size=ppr_idx[-1] + 1, reduce='sum')
        propagated_logits = propagated_logits / (ppr_sum_s + 1e-12)
        if self.use_bn:
            res = self.linear(self.dropout(self.bn(propagated_logits)))
        else:
            res = self.linear(self.dropout(propagated_logits))
        return res #propagated_logits / (ppr_sum_s + 1e-12)

    def lr(self, logits):
        if self.use_bn:
            res = self.linear(self.dropout(self.bn(logits)))
        else:
            res = self.linear(self.dropout(logits))
        return res
...
def predict(args, adj, features, model, idx_test, labels_org, inf_fraction=1.0, ppr_normalization='row', batch_size_logits=10000):
    model.eval()
    device=next(model.parameters()).device
    ...
    if ppr_normalization == 'sym':
        ...
        for _ in range(nprop):
            local_logits = (1 - args.alpha) * deg_sqrt_inv[:, None] * (
                adj @ (deg_sqrt_inv[:, None] * local_logits))
            logits += local_logits
    elif ppr_normalization == 'col':
        deg_col = adj.sum(0).A1
        deg_col_inv = 1. / np.maximum(deg_col, 1e-12)
        for _ in range(nprop):
            local_logits = (1 - args.alpha) * \
                (adj @ (deg_col_inv[:, None] * local_logits))
            logits += local_logits
            #(1 - args.alpha) * (adj @ (deg_col_inv[:, None] * logits)) + args.alpha * local_logits
        ...
        for _ in range(nprop):
            ...
            logits += local_logits
            norm_w_sum += norm_w * (1 - args.alpha)

    else:
        raise ValueError(f"Unknown PPR normalization: {ppr_normalization}")
    logits_tensor = torch.FloatTensor(logits)
    nnodes = logits_tensor.shape[0]
    pred_logits_list = []
    for i in range(0, nnodes, 1000):
        with torch.no_grad():
            pred_logits = model.lr(logits_tensor[i:i+1000].to(device))
            pred_logits_list.append(pred_logits)
    pred_logits_all = torch.cat(pred_logits_list)
    print(pred_logits_all.cpu().numpy().shape)
    preds=pred_logits_all.cpu().numpy().argmax(1)
    print(preds)            
...
def main_grandpp(args):
    ...
    for epoch in range(args.epochs):
        for train_index in iterate_minibatches_listinputs(idx_train, batch_size=args.batch_size, shuffle=True):
            ...
            batch_feat = features[neighbor_idx]
            ppr_scores = torch.tensor(ppr_scores, dtype=torch.float32)
            source_idx = torch.tensor(source_idx, dtype=torch.long)
            ...
            loss_train += min(1.0 , args.beta + ((1.0 - args.beta) * float(num_batch)/args.warmup)) * consis_loss([output_aug1,
                                      output_aug2], args.tem, args.lam,args.conf)

            acc_train = accuracy(output_aug1[:len(train_index)], y_train_batch)
            loss_train.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--unlabel_num', type=int,
                        default=-1, help='unlabeled node ratio')
