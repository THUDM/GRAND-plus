...
class MLP(nn.Module):
    def __init__(self, num_features, num_classes, hidden_size, nlayers, use_bn, dropout):
        ...
        if nlayers == 1:
            ...
            for i in range(nlayers - 2):
                fcs.append(weight_norm(nn.Linear(hidden_size, hidden_size, bias=False)))
    ...
    def forward(self, X):
        ...
        if self.use_bn:
            X = self.bns[0](X)
        embs = F.dropout(X, self.droprate, training=self.training)#.detach()
...
class Grand_MLP_Pre_Loss(nn.Module):
    ...
    def random_prop(self, feats, ppr_scores, ppr_idx):
        ppr_scores = F.dropout(ppr_scores, p=0.5, training=self.training)
        # nnode = feats.shape[0]
...
def valid(args, model, topk_adj, features, idx_val, labels, batch_size=10000):
    ...
    for idx in iterate_minibatches_listinputs(idx_val, batch_size):
        ...
        with torch.no_grad():
            val_feat = model.random_prop(val_feat, ppr_scores, source_idx).detach()
...
def predict(args, adj, features_np, model, idx_test, labels_org, mode='ppr', batch_size_logits=10000):
    ...
    if mode == 'ppr':
        ...
        for _ in range(nprop):
            features_np = deg_row_inv[:,None] * (adj @ features_np)
            features_np_prop += features_np
        logits = features_np_prop/(nprop + 1)
    else:
        raise ValueError(f"Unknown propagation mode: {mode}")
    
    logits = get_local_logits(
        model.mlp, features_np_prop, batch_size_logits)    
...
def main_grandpp(args):
    ...
    for epoch in range(args.epochs):
        for train_index in iterate_minibatches_listinputs(idx_train, batch_size=args.batch_size, shuffle=True):
            ...
            loss_train = 0.
            for i in range(K):
                batch_feat_aug = model.random_prop(batch_feat, ppr_scores, source_idx).detach()
    ...
    test_acc = predict(args, adj, features_np, model, idx_test, labels)
