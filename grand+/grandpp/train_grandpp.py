...
def predict(args, adj, features, model, idx_test, labels_org, inf_fraction=1.0, mode='ppr', batch_size_logits=10000):
    ...
    if inf_fraction < 1.0:
        idx_sub = np.random.choice(adj.shape[0], int(
            inf_fraction * adj.shape[0]), replace=False)
        idx_sub.sort()
        attr_sub = features[idx_sub]
        logits_sub = get_local_logits(model.mlp, attr_sub, batch_size_logits)
        local_logits = np.zeros(
            [adj.shape[0], logits_sub.shape[1]], dtype=np.float32)
